...
get_rectangle_vertices(const RigidBody &body,
    ...
    float width = body.shape_data.param.rectangle.width;
    float height = body.shape_data.param.rectangle.height;
...
get_triangle_vertices(const RigidBody &body,
                      const glm::vec3 &translate = glm::vec3(0.0, 0.0, 0.0),
                      const float rotate = 0.0) {
    float side = body.shape_data.param.triangle.side;
...
std::vector<glm::vec3> RigidBody::normals() const {
    ...
    default:
        throw std::runtime_error("Body must be a triangle");
    };
}
...
bool is_point_inside_rectangle(const RigidBody &body, WorldPoint &point) {
    ...
    float half_width = body.shape_data.param.rectangle.width / 2.0;
    float half_height = body.shape_data.param.rectangle.height / 2.0;
