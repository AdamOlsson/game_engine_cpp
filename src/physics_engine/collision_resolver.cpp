...
constexpr float BAUMGARTE_FACTOR = 0.8f;
...
resolve_collision_edge_edge(const CollisionInformation &ci, const RigidBody &body_a,
    ...
    // Compute positional change based on the deepest contact
    ...
    float total_impulse_magnitude = 0.0;
    float contact_count = 0.0;
    for (auto &contact_point : ci.contact_patch) {
        const glm::vec3 body_a_center_to_p = contact_point - body_a.position;
        ...
        const glm::vec3 body_b_center_to_p = contact_point - body_b.position;
        ...
        const float impulse_magnitude =
            calculate_impulse_magnitude(ci.normal, relative_vel_at_p, body_a, body_b,
                                        body_a_center_to_p, body_b_center_to_p, c_r);
        total_impulse_magnitude += impulse_magnitude;
        contact_count += 1.0;
    }

    const float average_impulse_magnutude = total_impulse_magnitude / contact_count;
    ...
    const glm::vec3 impulse = average_impulse_magnutude * ci.normal;
    for (auto &contact_point : ci.contact_patch) {
        correction_a.velocity +=
            velocity_correction(ci.normal, average_impulse_magnutude, body_a);
        correction_b.velocity +=
            velocity_correction(ci.normal, -average_impulse_magnutude, body_b);
        ...
        correction_b.angular_velocity +=
            angular_velocity_correction(body_b_center_to_p_perp, -impulse, body_b);
    }
...
std::optional<CollisionCorrections>
resolve_collision_vertex_vertex(const CollisionInformation &ci, const RigidBody &body_a,
                                const RigidBody &body_b) {
