/// This file is auto generated by compile_asset.py
#include "ui_fragment.h"

const std::string resource_name = "UIFragment";

unsigned char ui_fragment_spv[] = {
  
};
unsigned int ui_fragment_spv_len = 424;

std::unique_ptr<ShaderResource> UIFragment::create_resource() {
    return ShaderResourceBuilder()
        .name(std::move(resource_name))
        .length(ui_fragment_spv_len)
        .bytes(&ui_fragment_spv[0])
        .build();
}

