...
GraphicsPipeline::GraphicsPipeline(Window &window,
    ...
    triangleVertexBuffer =
        createVertexBuffer(ctx, Geometry::triangle_vertices, commandPool, graphicsQueue);
    triangleIndexBuffer =
        createIndexBuffer(ctx, Geometry::triangle_indices, commandPool, graphicsQueue);

    rectangleVertexBuffer =
        createVertexBuffer(ctx, Geometry::rectangle_vertices, commandPool, graphicsQueue);
    rectangleIndexBuffer =
        createIndexBuffer(ctx, Geometry::rectangle_indices, commandPool, graphicsQueue);
    ...
    for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
        triangleInstanceBuffers[i] = createStorageBuffer(ctx, size);
        rectangleInstanceBuffers[i] = createStorageBuffer(ctx, size);
