...
// CONTINUE: I need to also pass the max health when constructring the Health object
...
class Health {
  private:
    ...
    HealthBar create_health_bar(const math::Vector2 &position, const math::Vector2 &size,
                                const HealthBarOpts &opts) {
        ...
        instance.model_matrix = math::Matrix().translate(position).scale(size);
    ...
    void update_health_bar_(HealthBar &bar) {
        if (m_geometry_renderer != nullptr) {
            ...
            instance.model_matrix =
                math::Matrix()
                    .translate(m_health_bar_position + bar.offset)
  ...
  public:
    ...
    Health(graphics_pipeline::geometry::GeometryRenderer *geometry_renderer,
           const math::Vector2 &position, const math::Vector2 &size,
           const HealthBarOpts opts = {})
        : m_geometry_renderer(geometry_renderer), m_health_bar_position(position),
          m_health_bar_size(size) {
        m_health_pool.push_back(
            std::move(create_health_bar(m_health_bar_position, m_health_bar_size, opts)));
    }
    ...
    bool is_dead() const { return m_health_pool.back().current <= 0.0f; }
    ...
    void handle_incomming_damage(const float dmg) { m_health_pool.back().current -= dmg; }
