...
de_casteljaus(const CubicBezier &bezier) {
    const std::pair<int, int> A = lerp(bezier.p0, bezier.p1, 0.5f);
    const std::pair<int, int> B = lerp(bezier.p1, bezier.p2, 0.5f);
    const std::pair<int, int> C = lerp(bezier.p2, bezier.p3, 0.5f);

    const std::pair<int, int> D = lerp(A, B, 0.5f);
    const std::pair<int, int> E = lerp(B, C, 0.5f);

    const std::pair<int, int> F = lerp(D, E, 0.5f);
...
approximate_quadratic_bezier(const math::CubicBezier &cubic) {
    return math::QuadraticBezer{
        cubic.p0,
        std::make_pair((3.0f * (cubic.p1.first + cubic.p2.first) -
                        (cubic.p0.first + cubic.p3.first)) /
                           4.0f,
                       (3.0f * (cubic.p1.second + cubic.p2.second) -
                        (cubic.p0.second + cubic.p3.second)) /
                           4.0f),
        cubic.p3};
}
