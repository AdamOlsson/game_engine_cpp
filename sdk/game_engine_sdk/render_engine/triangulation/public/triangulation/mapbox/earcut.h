...
template <typename N = uint32_t> class Earcut {
  ...
  private:
    ...
    bool pointInTriangle(double ax, double ay, double bx, double by, double cx, double cy,
...
void Earcut<N>::operator()(const Polygon &points) {
    ...
    if (hashing) {
        ...
        // minX, minY and inv_size are later used to transform coords into integers for
        // z-order calculation
...
typename Earcut<N>::Node *Earcut<N>::findHoleBridge(Node *hole, Node *outerNode) {
    ...
    // endpoint; if there are no points found, we have a valid connection; otherwise
    // choose the Vertex of the minimum angle with the ray as connection Vertex
    ...
    do {
        if (hx >= p->x && p->x >= mx && hx != p->x &&
            pointInTriangle(hy < my ? hx : qx, hy, mx, my, hy < my ? qx : hx, hy, p->x,
                            p->y)) {
...
template <typename N>
bool Earcut<N>::pointInTriangle(double ax, double ay, double bx, double by, double cx,
...
// create a node and util::optionally link it with previous one (in a circular doubly
// linked list)
template <typename N>
