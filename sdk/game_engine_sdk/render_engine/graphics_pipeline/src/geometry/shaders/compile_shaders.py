...
def main():
    ...
    print("Compiling geometry shaders...")
    
    # Compile vertex shader
    if not compile_shader(glslc_path, script_dir, "geometry_vertex", "vert"):
        sys.exit(1)
    
    # Compile fragment shader
    if not compile_shader(glslc_path, script_dir, "geometry_fragment", "frag"):
        sys.exit(1)
    
    # Run xxd on compiled shaders
    print("Converting geometry shaders to CPP files...")
    # Convert vertex shader
    conf = {
        "location": script_dir,
        "input_file": "geometry_vertex.spv",
        "output_cpp_file": "geometry_vertex_shader.cpp",
        "output_h_file": "geometry_vertex_shader.h",
        "resource_name": "GeometryVertexShader",
        "xxd_variable_name": "geometry_vertex_bytes",
        "class_name": "GeometryVertexShader",
    }
    if not run_xxd(conf):
        sys.exit(1)
    ...
    conf = {
        "location": script_dir,
        "input_file": "geometry_fragment.spv",
        "output_cpp_file": "geometry_fragment_shader.cpp",
        "output_h_file": "geometry_fragment_shader.h",
        "resource_name": "GeometryFragmentShader",
        "xxd_variable_name": "geometry_fragment_bytes",
        "class_name": "GeometryFragmentShader",
    }
    if not run_xxd(conf):
        sys.exit(1)
    
    print("Compiling geometry shaders... OK!")
    return 0
