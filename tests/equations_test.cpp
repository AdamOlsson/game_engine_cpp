...
TEST(EquationsTest, Given2dVectorExpectRoundedTo3Decimals) {
    ...
    EXPECT_EQ(expected, output)
        << "Expected " << vec_to_string(expected) << " found " << vec_to_string(output);
}
...
TEST(EquationsTest, RoundMut2dVectorProducesSameResultAsRound) {
    ...
    Round::round_mut(v2);
    EXPECT_EQ(output1, v2) << "Expected " << vec_to_string(output1) << " found "
                           << vec_to_string(v2);
}
...
TEST(EquationsTest, RotateMut2dVecShouldProduceSameResultAsRotate) {
    ...
    EXPECT_EQ(output1, output2)
        << "Expected " << vec_to_string(output1) << " found " << vec_to_string(output2);
}
